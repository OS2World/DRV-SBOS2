...
/*  new = malloc (strlen (s) + 1); */
  ...
  rc=DosAllocMem( (PVOID *) &new, strlen(s)+1, fALLOC);
  if (rc)
    printf("Error allocating memory in getstring = %ld\n",rc);
...
fill_sample_info (struct sample_info *info, FILE *f)
{
  ...
  rc=DosAllocMem( (PVOID *) &info->start, info->length, fALLOC);

  if (rc)
    printf("Error in DosAllocMem = %ld\n",rc);
...
/*  new = (struct song *) malloc (sizeof (struct song));*/
  ...
  rc=DosAllocMem( (PVOID *) &new, sizeof(struct song), fALLOC );
  if (rc)
    printf("Error in new_song allocmem = %ld\n",rc);
...
/*  new = (struct sample_info *) malloc (sizeof (struct sample_info));*/
  rc=DosAllocMem( (PVOID *) &new, sizeof(struct sample_info), fALLOC);
  if (rc)
    printf("Error alloc mem in new_sample_info =%ld\n",rc);
...
/*  new = (struct song_info *) malloc (sizeof (struct song_info)); */
  rc=DosAllocMem( (PVOID *) &new, sizeof(struct song_info), fALLOC);
  if (rc)
    printf("Error alloc mem in new song info = %d\n",rc);
  ...
  for (i = 0; i < 31; i++)
      ...
      if (song->samples[i])
	{
	  if (song->samples[i]->start)
	    ...
	    {
	      rc=DosFreeMem(song->samples[i]->start);
	      if (rc)
		printf("Error deallocating 1 = %ld\n",rc);
	    }
	  if (song->samples[i]->name)
	    /* free (song->samples[i]->name); */
	    {
	      rc=DosFreeMem(song->samples[i]->name);
	      if (rc)
		printf("Error deallocating 2 = %ld\n",rc);
	    }

	  /* free (song->samples[i]); */
	  rc=DosFreeMem(song->samples[i]);
	  if (rc)
	    printf("Error deallocating 3 = %ld\n",rc);
	}
    }
  if (song->info)
    {
	...
	{
	  rc=DosFreeMem(song->info->pblocks);
	  if (rc)
	    printf("Error deallocating 4 = %ld\n",rc);
	}

      /* free (song->info); */
      rc=DosFreeMem(song->info);
      if (rc)
	printf("Error deallocating 5 = %ld\n",rc);

    }
  if (song->title)
    /* free (song->title);*/
    {
      rc=DosFreeMem(song->title);
      if (rc)
	printf("Error deallocating 6 = %ld\n",rc);
    }

  /* free (song);*/
  rc=DosFreeMem(song);
  if (rc)
    printf("Error deallocating 7 = %ld\n",rc);
...
read_song (FILE *f, int type, int tr)
{
  ...
  if (DosAllocMem( (PVOID *) &(song->info->pblocks), 
		  sizeof (struct block) * song->info->maxpat,
		  fALLOC))
    printf("Error alloc mem in read_song\n");
